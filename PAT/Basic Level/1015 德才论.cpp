...
struct student{
	...
	bool operator < (student p) const {
		if (de + cai == p.de + p.cai) {
			if (de == p.de) return id < p.id;
			return de > p.de;
		}
		return de + cai > p.de + p.cai;
	}
};
...
int main(){
    ...
    for(int i  = 0; i < n; i++){
        ...
        if(de >= l && cai >= l){
            cnt ++;
            if(de >= h && cai >= h){
                vs[0].push_back({id,de,cai});
            }else if( de >= h && cai < h){
                vs[1].push_back({id,de,cai});
            }else if(de < h && cai < h && de >= cai){
                vs[2].push_back({id,de,cai});
            }else{
                vs[3].push_back({id,de,cai});
            }
        }
    }
    printf("%d\n",cnt);
    for(int i = 0; i < 4; i++){
        ...
        for(int j = 0; j < vs[i].size(); j++){
            printf("%08d %d %d\n", vs[i][j].id, vs[i][j].de, vs[i][j].cai);
        }
    }
    return 0;
}